...
import { dataQuestionsA, dataQuestionsB } from './dummy-data';
...
export class SectionTwoComponent implements OnInit {
  ...
  getListQuestion() {
    ...
    if(character == '1') {
      this.listQuestions = dataQuestionsA;
    } else {
      this.listQuestions = dataQuestionsB;
    };
  ...
  goToNextSection() {
    ...
    UtilsTempoProgress.setFinish(1);
  ...
  checkToggle() {
    this.questionService.togglePanel.subscribe({
      next: (res) => {
        this.togglePanel = res;
        // this.width = this.togglePanel ? 270 : 70;
      }
    });
  }
}
